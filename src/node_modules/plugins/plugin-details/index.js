...
function getTemplateDetails({ data = [], placeholder = 'placeholder пустой', selectedValue }) {
  const items = data.map(item => {
    ...
    return `<a href="#" class="list-group-item list-group-item-action py-2 ${cls}" data-type="item" data-value="${item.value}">${item.value}</a>`
  })
  ...
  return `
    ...
    <div class="details-list list-group position-absolute top-100 start-0 end-0 shadow mt-2">${items.join('')}</div>`
}
